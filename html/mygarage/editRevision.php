...
if ($revision_id) {
    ...
    $error_message = "Richiesta non valida a causa di revision_id non fornito.";
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($amount) || empty($buying_date)) {
        ...
        $sql = "UPDATE vehicle_revisions 
                SET amount = ?, buying_date = ? 
                WHERE id = ? AND user_id = ?";
        if ($stmt = $link->prepare($sql)) {
            $stmt->bind_param("ssii", $amount, $buying_date, $revision_id, $user_id);
            if ($stmt->execute()) {
