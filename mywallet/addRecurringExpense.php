...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($name) || empty($amount) || empty($start_month) || empty($start_year) || empty($debit_date) || empty($billing_frequency)) {
        ...
        $check_sql = "SELECT id FROM recurring_expenses WHERE user_id = ? AND name = ? AND amount = ? AND start_month = ? AND start_year = ? AND (end_month = ? OR end_month IS NULL) AND (end_year = ? OR end_year IS NULL) AND undetermined = ? AND debit_date = ? AND billing_frequency = ?";
        if ($check_stmt = $link->prepare($check_sql)) {
            ...
            if ($check_stmt->num_rows > 0) {
                ...
                $sql = "INSERT INTO recurring_expenses (user_id, name, amount, start_month, start_year, end_month, end_year, undetermined, debit_date, billing_frequency) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
